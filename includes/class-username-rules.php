...
/**
 * Username heuristics: min/max length, block numeric-only, block obvious spam patterns.
 */
class BPRA_UsernameRules {
    ...
    public function validate() {
        ...
        $max    = (int) bpra_setting( 'max_username_length', 20 );
        ...
        } elseif ( mb_strlen( $user ) > $max ) {
            $errors['signup_username'] = sprintf( __( 'Username cannot exceed %d characters.', 'bpra' ), $max );
            BPRA_Logger::log( 'username_long', array( 'user' => $user ) );
